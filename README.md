...
### 主な機能 
#### 【Core】 
- Unity に依存しない「暗号化・永続化・メッセージング・リスト等の共通ユーティリティ」と、それらの抽象インターフェース。

#### 【Unity】
- Unityに依存をする Core の抽象を実装した「ファイル IO、暗号化付きストレージ、階層メッセージング、コンポーネント拡張」。
...
Package Managerの「Add package from git URL...」から以下のURLを入力してインストールしてください。
```
https://github.com/chinpanGX/Supplement.git?path=Assets/Supplement.ZeroMessanger
```
